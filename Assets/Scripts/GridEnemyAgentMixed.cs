...
/// <summary>
/// Chế độ MIXED — kết hợp A* và PIBT trong một agent.
///
/// Trực giác: "Phối hợp ở xa, tham lam ở gần" (coordinated far, greedy near).
///  - Khi còn XA Eagle: dùng PIBT (flow-aware) để tản luồng qua bottleneck/cửa hẹp,
///    tránh cả đàn dồn cùng một cửa → ít kẹt, ít recovery.
///  - Khi đã GẦN Eagle: mọi trajectory bắt buộc hội tụ về đúng ô Eagle nên flow không
///    còn giúp được (vertex_flow tất yếu cực đại) mà chỉ sinh dao động; chuyển sang A*
///    để lao thẳng vào — rẻ và chính xác. Đây cũng là vùng eagleShootingRange.
///
/// Tiêu chí switch: khoảng cách lưới còn lại tới Eagle (reverse-BFS đã cache trong
/// PIBTPlanner) + hysteresis (switchMargin) chống rung ở ranh giới.
///
/// Đây là bản sao GridEnemyAgentPIBT; khác biệt DUY NHẤT nằm ở ReplanPath() (chọn
/// pathfinder theo pha) + UpdatePhase(). Follow-path/recovery/shooting giữ nguyên.
/// Khi ở pha A*, agent gọi PIBTPlanner.SuspendAgent() để rút flow ra, không bơm "flow
/// ma" làm lệch đường của đồng đội đang chạy PIBT.
/// </summary>
public class GridEnemyAgentMixed : MonoBehaviour
{
    ...
    // PIBT = phối hợp ở xa, AStar = tiếp cận cục bộ ở gần Eagle.
    private enum Phase
    {
        PIBT,
        AStar
    ...
    [Header("Mixed switch")]
    [Min(1)]
    [Tooltip("Bán kính (ô) quanh Eagle mà agent chuyển từ PIBT sang A*. Nên lớn hơn tầm bắn để đoạn cuối đã là A* trước khi dừng bắn.")]
    public int switchRadiusCells = 8;
    [Min(0)]
    [Tooltip("Hysteresis: chỉ chuyển A*→PIBT khi khoảng cách vượt (switchRadiusCells + margin). Chống rung ở ranh giới.")]
    public int switchMargin = 3;
    ...
    private Phase _phase = Phase.PIBT;
    ...
    [System.NonSerialized] public int   btSwitchCount;   // số lần đổi pha PIBT<->A*
    ...
    // ── Pathfinding (MIXED: PIBT ở xa, A* ở gần) ───────────────────────────
    ...
    // Cập nhật pha theo khoảng cách lưới còn lại (có hysteresis). Lần đầu chỉ phân loại,
    // không tính là switch.
    private void UpdatePhase(int distToGoal)
    {
        Phase target;
        if (!_hasPhase)
        {
            _phase = distToGoal <= switchRadiusCells ? Phase.AStar : Phase.PIBT;
            _hasPhase = true;
            return;
        }

        if (_phase == Phase.PIBT)
            target = distToGoal <= switchRadiusCells ? Phase.AStar : Phase.PIBT;
        else // AStar → chỉ quay lại PIBT khi vượt ngưỡng + margin (chống rung)
            target = distToGoal >= switchRadiusCells + switchMargin ? Phase.PIBT : Phase.AStar;
    ...
    private void ReplanPath()
    {
        ...
        // ── Chọn pha: PIBT khi còn xa, A* khi đã gần Eagle ──────────────────
        int distToGoal = PIBTPlanner.GetHeuristicDistance(startCell, goalCell);
        UpdatePhase(distToGoal);

        bool found;
        if (_phase == Phase.PIBT)
        {
            // Phối hợp: bơm flow + FrankWolfe replan hộ đồng đội trong ngân sách thời gian.
            found = GridPIBTPathfinder.TryFindPath(mapLoader, agentId, startCell, goalCell, currentPath, frankWolfeMs);
        }
        else // Phase.AStar — tiếp cận cục bộ
        {
            // Rút khỏi tập active PIBT: bỏ flow ma + không bị FrankWolfe replan hộ.
            PIBTPlanner.SuspendAgent(agentId);
            // A* "vật lý" (navMask=null) giống nhánh destructible; tránh cả ô đồng đội chiếm.
            found = GridAStarPathfinder.TryFindPath(mapLoader, null, startCell, goalCell, currentPath, blockedCells);
        }
    ...
    private bool TryTriggerScuffRecovery()
    {
        ...
        Debug.Log($"[GridEnemyAgentMixed] {name} scuff recovery triggered.");
    ...
    private void TriggerRecovery()
    {
        ...
        switch (recoveryLevel)
        {
            ...
            case RecoveryLevel.Reverse:
                // Sau reverse → replan lại, reset về None (PIBT/A* sẽ tự tìm đường mới)
                recoveryLevel = RecoveryLevel.None;
                currentPath.Clear();
                pathIndex = 0;
                nextReplanTime = 0f;
                ReplanPath();
                Debug.LogWarning($"[GridEnemyAgentMixed] {name} recovery: replan after reverse.");
                break;
        }
    ...
    private void OnDrawGizmos()
    {
        ...
        // Cyan = pha PIBT (phối hợp ở xa), Đỏ = pha A* (tiếp cận cục bộ ở gần Eagle).
        Gizmos.color = _phase == Phase.AStar ? Color.red : Color.cyan;
