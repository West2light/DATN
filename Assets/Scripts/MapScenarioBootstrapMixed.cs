...
/// <summary>
/// Bootstrap cho Mode 4: MIXED (kết hợp A* + PIBT).
///
/// Bản sao MapScenarioBootstrapPIBT nhưng gắn GridEnemyAgentMixed thay GridEnemyAgentPIBT
/// và truyền thêm tham số switch (switchRadiusCells/switchMargin). Dùng chung scene
/// MapF_TankTest_PIBT (Mixed không cần GridNavMask, giống PIBT).
///
/// Gắn component này cùng chỗ với MapLoader; MapTankTestBootstrap add lúc runtime khi
/// algorithm = "Mixed" (giống cách xử lý PIBT_TCP).
/// </summary>
public class MapScenarioBootstrapMixed : MonoBehaviour
{
    ...
    [Min(1)]
    [Tooltip("Enemy health in the Mixed scene, matching Lvl1.")]
    public int enemyMaxHealth = 20;

    [Header("PIBT")]
    [Tooltip("Time budget (ms) for Frank-Wolfe iterations on each PIBT-phase replan.")]
    [Min(1f)] public float frankWolfeMs = 25f;

    [Header("Mixed switch")]
    [Min(1)]
    [Tooltip("Bán kính (ô) quanh Eagle mà agent chuyển từ PIBT sang A*.")]
    public int switchRadiusCells = 8;
    [Min(0)]
    [Tooltip("Hysteresis: chỉ chuyển A*→PIBT khi khoảng cách vượt (switchRadiusCells + margin).")]
    ...
    public void SpawnScenario()
    {
        ...
        {
            Debug.LogError("[MapScenarioBootstrapMixed] Map must be loaded before spawning scenario.");
            return;
        }
        ...
        ClearScenario();
        scenarioRoot = new GameObject("ScenarioRuntime_Mixed").transform;
    ...
    private GameObject SpawnEagleBase()
    {
        ...
        {
            Debug.LogError("[MapScenarioBootstrapMixed] Could not find walkable Eagle spawn cell.");
            return null;
        }
    ...
    private void SpawnEnemies()
    {
        ...
        if (prefab == null)
        {
            Debug.LogError("[MapScenarioBootstrapMixed] Enemy prefab is missing.");
            return;
        }
    ...
    private void AddGridEnemyAgentMixed(GameObject enemy)
    {
        ...
        agent.switchRadiusCells = switchRadiusCells;
        agent.switchMargin = switchMargin;
    ...
    {
        enemies.Clear();
        Transform existing = transform.Find("ScenarioRuntime_Mixed");
