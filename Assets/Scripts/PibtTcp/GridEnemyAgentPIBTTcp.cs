...
///   3. Mỗi agent đọc action theo agentId và drive TankController
///
...
/// </summary>
public class GridEnemyAgentPIBTTcp : MonoBehaviour
{
    ...
    private void SteerTowardTargetCell(Vector2 directionToTarget)
    {
        ...
        float dotProduct = Vector2.Dot(forward, directionNormalized);
        ...
        {
            ResetPartialDrive();
            float forwardDrive = IsScuffing() ? 0f : 0.1f;
            tankController.HandleMoveBody(new Vector2(rotation, forwardDrive));
        }
    }
    ...
    private TankMover GetTankMover() => tankController != null ? tankController.tankMover : null;
    ...
    private void TraceAgentExecution(string phase, ActionDto dto)
    {
        ...
        Debug.Log(
            ...
            $"cell={currentCell} goal={eagleCell} assignedGoal={assignedGoalText} canShootEagle={canShootEagle} shootTarget={shootingTargetText} facing={committedFacing} physFacing={QuantizeFacingFromTransform()} pos=({pos.x:F2},{pos.y:F2}) " +
