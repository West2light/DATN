...
% XeLaTeX: dùng fontspec thay vietnam+times để hỗ trợ Unicode tiếng Việt đầy đủ
\usepackage{fontspec}
\setmainfont{Times New Roman}
% Việt hóa các nhãn mặc định (do không dùng gói vietnam nữa)
...
% times thay bằng fontspec/setmainfont ở trên
...
\usepackage{longtable} % Bảng đặc tả use case in-flow, tự ngắt trang (tránh khoảng trắng do float)
...
% Cho phép ngắt dòng URL dài trong danh mục tài liệu tham khảo (tránh tràn lề phải)
...
\def \TITLE{Ứng dụng bài toán Multi-Agent Pathfinding trong game bắn xe tăng nhiều người chơi}
\def \AUTHOR{Dương Quang Đông}
...
% Last-resort: cho phép giãn nhẹ khoảng cách từ ở các dòng có từ/định danh quá dài,
% kết hợp với \allowbreak để tránh chữ tràn ra lề phải (overfull \hbox)
...
% Số trang phần đầu (chữ số La Mã) căn sát lề phải ở footer, đồng nhất với phần thân
\fancypagestyle{plain}{%
  \fancyhf{}%
  \fancyfoot[R]{\thepage}%
  \renewcommand{\headrulewidth}{0pt}%
...
\pagestyle{empty} % Mục lục: KHÔNG hiện số trang (số La Mã chỉ bắt đầu từ Danh mục hình vẽ)
...
% Bắt đầu đánh số trang La Mã (căn sát lề phải) từ Danh mục hình vẽ trở đi
\pagenumbering{roman}
\pagestyle{plain}
...
\pagestyle{fancy}
\fancyhf{}
% Header chương: trang lẻ canh trái, trang chẵn canh phải (kiểu sách 2 mặt) + kẻ ngang dưới
\fancyhead[L]{\ifodd\value{page}\makebox[\headwidth][l]{\small\leftmark}\else\makebox[\headwidth][r]{\small\leftmark}\fi}
\fancyfoot[R]{\thepage}
...
\titlecontents{chapter}
    ...
    {}         % unnumbered format
    {\titlerule*[0.3pc]{.}\contentspage}         % filler-page-format, e.g dots
\chapter{KẾT QUẢ BACKTEST CHI TIẾT}
\subfile{Chuong/Phu_luc_A}
\newpage
\chapter{NHÓM USE CASE TRIỂN KHAI VÀ VẬN HÀNH}
