...
environments, demanding fast, real-time replanning as the environment changes. Traditional
...
PIBT C++ server; (iv) an EPIBT-based rotation-handling improvement that removes the
90-degree in-place snapping of agents; and (v) an automated backtest system supporting
static and dynamic-obstacle environments, exporting per-run CSV metrics. Experiments across
5 maps, 3 algorithms, and 5 agent-count levels (6, 12, 24, 36, 72) with 6 repetitions per
combination over both static and dynamic environments (900 runs in total)
...
mechanism; completion time and timeout rate grow with the number of agents; and enabling
dynamic obstacles increases replanning by 30--50\% on average while the overall mission
